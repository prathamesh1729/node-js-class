...
// Lookup all checks, get data, & send to validator
			...
			checks.forEach(function(check) {
				...
				_data.read('checks', check, function(err, originalCheckData) {
					if (!err && originalCheckData) {
						// Pass the data to the check validator & let that function continue & log errors as needed
...
workers.performCheck = function(originalCheckData) {
	...
	// Parse the hostname & hte path out of the original check data 
	var parsedUrl = url.parse(originalCheckData.protocol + '://' + originalCheckData.url, true);
	var hostName = parsedUrl.hostname;
	var path = parsedUrl.path; // Using path and not pathname because we want the entire query string
	...
	// Bind to the error event so it doesnt throw
	req.on('error', function(e) {
		// Update the checkout & pass data
		checkOutcome.error = {
			'error': true,
			'value': e
		};
	...
	// Bind to the timeout event so it doesnt throw
...
// Process the check outcome and update the check data as needed and trigger aler to user as needed
...
workers.processCheckOutcome = function(originalCheckData, checkOutcome) {
	...
	// Update the check data back in the db
...
workers.rotateLogs = function() {
	...
	// List all the non compressed files sitting in the .logs folder
	_logs.list(false, function(err, logs) {
		if (!err && logs && logs.length > 0) {
			logs.forEach(function(logName) {
				// Compres data to a different file
				...
				_logs.compress(logId, newFileId, function(err) {
					if (!err) {
						...
						debug("Error compressing on of the files", err);
					}
				});
			});
		} else {
