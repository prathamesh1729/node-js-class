 ...
 * ORDERS
 */
apis.orders = function(data, callback) {
  var acceptableMethods = ['get', 'post', 'put', 'delete'];
  if (acceptableMethods.indexOf(data.method) > -1) {
    apis._orders[data.method](data, callback);
  } else {
    callback(405);
  }
};
...
// Required fields: None
// Present items in cart will be converted to a order
// If cart is empty appropriate error will be returned
apis._orders.post = function(data, callback) {
  var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
  if (token) {
    apis._tokens.validateToken(token, function(tokenIsValid, tokenData) {
      if (tokenIsValid) {
        // Fetch users current cart and convert it into an order
        _data.read('pusers', tokenData.email, function(err, userData) {
          if (!err && userData) {
            // Successfully fetched the user data. Check if the cart has any items in it
            if (!helpers.isObjectEmpty(userData.cart)) {
              // Create the order
              ...
              var expandedCart = _menu.expand(userData.cart);
              var cartTotal = helpers.computeCartTotal(expandedCart);
              ...
              // Save the order
              _data.create('porders', orderId, orderObject, function(err) {
                if (!err) {
                  // Add the order to user data & empty the cart
                  userData.orders = userOrders;
                  userData.orders.push(orderId);
                  userData.cart = {};

                  // Save back the user with order info updated
                  _data.update('pusers', tokenData.email, userData, function(err) {
                    if (!err) {
                      // All good. Return 200, with the newly placed order object
                      // callback(200, orderObject);

                      // Attempt to charge here
                      var description = 'Order #' + orderId;
                      helpers.chargePayment(cartTotal * 100, config.stripe.source, config.stripe.currency, description, orderObject, function(err) {
                        if (!err) {
                          // Card payment successful! All good
                          // callback(200, orderObject);
                          orderObject.paid = true;

                          // Update the order payment status
                          _data.update('porders', orderId, orderObject, function(err) {
                            if (!err) {

                              // Attempt to send an email via mailgun
                              helpers.sendEmail(tokenData.email, 'Order Placed!', 'Your order has been placed: ' + JSON.stringify(orderObject), function(err) {
                                if (!err) {
                                  // Payment done, email sent. All good
                                  callback(200, orderObject);
                                } else {
                                  callback(200, {'Success': 'Payment processed & order placed but could not send confirmation email', 'Order': orderObject});
                                }
                              });

                            } else {
                              callback(500, {'Error': 'Payment processed & order placed with ID #' + orderId + ' but payment status could not be updated. Please contact admin'});
                            }
                          });

                        } else {
                          callback(400, {'Error': 'Payment could not be processed for the order', 'Order': orderObject});
                        }
                      });
                  ...
                  callback(500, {'Error': 'Failed to create the order'});
                }
              });

            } else {
              callback(400, {'Error': 'No items in cart to checkout'});
            }
